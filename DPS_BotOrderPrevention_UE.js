...
define(['N/record', 'N/runtime', 'N/search', 'N/email', 'N/format'],
    function (record, runtime, search, email, format) {
        ...
        // Configuration
        const MAX_ORDERS_PER_HOUR = 1;  // Adjust based on your normal traffic patterns
                ...
                try {
                    ...
                    if (runtime.executionContext === runtime.ContextType.WEBSERVICES || runtime.executionContext === runtime.ContextType.CUSTOMER_CENTER || runtime.executionContext === runtime.ContextType.USER_INTERFACE) {
                        ...
                        if (orderCount >= MAX_ORDERS_PER_HOUR) {
                            ...
                            log.debug('ipAddress set value', ipAddress)
                            log.debug('orderCount set value', orderCount)
                            ...
                            logRec.setValue('custrecord_dps_detection_timestamp', new Date())
                            logRec.setValue('custrecord_dps_ip_address', ipAddress)
                            logRec.setValue('custrecord_dps_order_count', orderCount)
                            logRec.setValue('custrecord_dps_detection_type', 'Excessive Orders')
                            var logRecID = logRec.save({ enableSourcing: true, ignoreMandatoryFields: true });
